...
function twitter_messages($username = '', $num = 1, $list = false, $update = true, $linked  = '#', $hyperlinks = true, $twitter_users = true, $encode_utf8 = false) {
	...
	$cache_ttl = 3600;
	$cache_file = WP_PLUGIN_DIR.'/twitter-for-wordpress/cache.dat';
	$from_cache = true;
	$cache_exists = (file_exists($cache_file) && filesize($cache_file)>0);

	if (!$cache_exists || ((filemtime($cache_file) + $cache_ttl) < time())) {
		$messages = fetch_rss('http://twitter.com/statuses/user_timeline/'.$username.'.rss');
		if (!empty($messages->items)) {
			file_put_contents($cache_file,serialize($messages));
			$from_cache = false;
		}
	}

	if ($from_cache && $cache_exists) {
		$messages = unserialize(file_get_contents($cache_file));
	}
