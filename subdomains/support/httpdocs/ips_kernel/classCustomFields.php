 ...
 * Last Updated: $Date: 2010-06-28 20:04:19 -0400 (Mon, 28 Jun 2010) $
 * </pre>
 *
...
class customFieldCbox extends customFieldPlugin implements interfaceCustomFieldPlugin
{
	...
	public function edit()
	{
		...
		foreach( $carray as $entry )
		{
			...
			{
				$this->parsed .= "<p><input type='checkbox' class='input_check' id='{$this->name}_{$ov}' name='{$this->name}[$ov]' value='1' checked='checked'{$_attributes}/> $td</p>\n";
			}
			else
			{
				$this->parsed .= "<p><input type='checkbox' class='input_check' id='{$this->name}_{$ov}' name='{$this->name}[$ov]' value='1'{$_attributes}/> $td</p>\n";
			}
		}
	}
...
class classCustomFields
{
	...
	/**
	 * Returns an array of supported field search types
	 *
	 * @access	public
	 * @return	array
	 **/	
