 ...
 * Last Updated: $Date: 2010-07-06 18:05:41 -0400 (Tue, 06 Jul 2010) $
 * </pre>
 *
...
class classTemplate
{
	...
	*/
	 ...
	 * @var		string
	 */
	private $_dqReplacement = '{{^|^escapedDQ^|^}}';
...
EOF;
	...
	*/
	public function convertHtmlToPhp($func_name, $func_data, $func_html, $func_desc="", $com_bit_update_trigger='', $compile=1)
	{
		...
		if( is_array( $this->_hookIds[ $this->_skinGroup ][ $func_name ] ) )
		{
...
EOF;
			...
			{
				$hookPHP .= "\$this->functionData['{$func_name}'][{$__count}]['{$v}'] = func_get_arg( $k );\n";
			}
...
//	print_r( $this->foreach_blocks );exit();
		return str_replace("\\'", "'", $template);
	}
