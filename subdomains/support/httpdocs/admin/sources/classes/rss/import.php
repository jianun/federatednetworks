...
class rss_import
{
	 ...
	 * @var		string
	 */
	private $_settings = array( 'AuthUser'    => '',
								'AuthPass'    => '',
								'ParseBbcode' => 1,
								'ParseHtml'   => 0,
	...
	public function __construct( $url, $key=false )
	{
		...
		if ( ! is_object( $this->_rssClass ) )
		{
			...
			$this->_rssClass               =  new classRss();
			
			$this->_rssClass->use_sockets  =  $this->use_sockets;
	...
	public function __call( $method, $arguments )
	{
		...
		if ( array_key_exists( $theRest, $this->_settings ) )
		{
			if ( $firstBit == 'set' )
			{
		...
		foreach ( $this->_rssClass->rss_channels as $channel_id => $channel_data )
		{
			if ( is_array( $this->_rssClass->rss_items[ $channel_id ] ) and count ($this->_rssClass->rss_items[ $channel_id ] ) )
			{
				...
				foreach( $this->_rssClass->rss_items[ $channel_id ] as $item_data )
				{
					...
					/*  Convert char set? */
					if ( $this->_classRss->orig_doc_type AND ( strtolower(IPS_DOC_CHAR_SET) != $this->_classRss->orig_doc_type) )
					{
						$item_data['title']   = IPSText::convertCharsets( $item_data['title']  , $this->_classRss->orig_doc_type, IPS_DOC_CHAR_SET );
						$item_data['content'] = IPSText::convertCharsets( $item_data['content'], $this->_classRss->orig_doc_type, IPS_DOC_CHAR_SET );
					}
	...
	public function parseContent( $content, $link="" )
	{
		...
		if ( ! $this->_settings['ParseHtml'] )
		{
			...
			$content = IPSText::getTextClass( 'bbcode' )->preDbParse( IPSText::getTextClass( 'editor' )->processRawPost( $_raw ) );
		}
		else
		{
		...
		if ( $this->_settings['LinkBack'] AND $link )
		{
			...
			if ( $this->_settings['ParseHtml'] )
			{
				...
				$the_link = "<br /><br />" . IPSText::getTextClass( 'bbcode' )->preDbParse( IPSText::getTextClass( 'editor' )->processRawPost( $_raw ) );
			}
			else
			{
	...
	private function _reset()
	{
		...
		$this->_rssClass->feed_charset 	= IPS_DOC_CHAR_SET;
