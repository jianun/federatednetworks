 ...
 * Last Updated: $Date: 2010-06-25 10:11:53 -0400 (Fri, 25 Jun 2010) $
 * </pre>
 *
 * @author 		$Author: matt $
...
class classItemMarking
{
	 ...
	 */
	private $_canCompressCookies = false;
	
	/**
	 * Disable all marking for guests
	 */
	private $_noGuestMarking     = false;
	...
	*/
	private function _compressCookie( $data )
	{
	...
	*/
	private function _inflateCookie( $data )
	{
		/* already inflated? */
	...
	public function markRead( $data, $app='' )
	{
		...
		if ( $data['itemID'] )
		{
			...
			/* Fetch unread items */
			if ( $this->settings['topic_marking_enable'] OR ( ! $this->settings['topic_marking_enable'] AND ! $this->settings['topic_marking_noquery'] ) )
			{
	...
	public function markAppAsRead( $app='' )
	{
		...
		else
		{
			...
			/* Bug #23130 If DB marking is off, _itemMarkers is empty, so we just assume forums */
			if ( ! count( $this->_itemMarkers ) )
			{
				$this->_itemMarkers = array( 'forums' => array() );
			}
			
			foreach( $this->_itemMarkers as $app => $data )
			{
				/* Only process is the app actually does topic marking */
				$f = IPSLib::getAppDir( $app ) . '/extensions/coreExtensions.php';
			
				if ( file_exists( $f ) )
				{
					require_once( $f );
					
					if ( ! @class_exists( 'itemMarking__' . $app ) )
					{
						continue;
					}
				}
					...
					foreach( $itemMarkers as $app => $data )
					{
						foreach( $itemMarkers[ $app ] as $key => $data )
						{
							if ( $key AND $app AND $memberID )
							{
								...
								if ( ! $new_array['item_global_reset'] AND ! $new_array['item_unread_count'] AND ( ! $new_array['item_read_array'] OR $new_array['item_read_array'] == 'a:0:{}' ) )
								{
									$this->DB->update( 'core_item_markers', array( 'item_is_deleted' => 1 ), "item_key='{$key}' AND item_app='{$app}' AND item_member_id=" . $memberID );
	...
	public function __myDestruct()
	{
		...
		if ( $this->settings['topic_marking_enable'] )
		{
			...
			if ( $update )
			{
				/* Delete flagged items using index */
				$this->DB->delete( 'core_item_markers', 'item_member_id > 0 AND item_is_deleted=1' );
	...
	protected function _generateIncomingItemMarkers()
	{
		...
		if ( ! is_array( $items ) OR ! count( $items ) )
		{
			...
			$this->DB->build( array( 'select' => '*',
									 ...
									 'where'  => 'item_member_id=' . $this->memberData['member_id'] . ' AND item_is_deleted=0' ) );
