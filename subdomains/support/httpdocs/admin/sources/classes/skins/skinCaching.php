	...
	public function rebuildPHPTemplates( $setID, $groupOnly='' )
	{
		...
		foreach( $skinSetData['_childTree'] as $_setID )
		{
			...
			foreach ( $groupTitles as $name => $group )
			{
				...
				$fileHead  = "class {$group['template_group']}_{$_setID} {\n\n";
				$fileHead .= <<<EOF
/**
* Construct
*/
...
EOF;
	...
	public function rebuildMasterReplacements( $setId=0 )
	{
		...
		if ( ! file_exists( $dir . '/_replacements.inc' ) )
		{
			return FALSE;
		}
		...
		foreach( $replacements as $k => $v )
		{
			...
			$this->DB->insert( 'skin_replacements', array( 'replacement_key'        => $k,
							   							   ...
							   							   'replacement_master_key' => ( is_numeric( $setId ) ) ? '' : $setId,
				...
				if ( ! $file->isDot() AND ! $file->isDir() )
				{
					...
					if ( substr( $_name, -4 ) == '.css' )
					{
						...
						$this->DB->insert( 'skin_css', array( 'css_updated'    => time(),
															  'css_set_id'     => ( is_numeric( $setId ) ) ? $setId : 0,
															  ...
															  'css_master_key' => ( is_numeric( $setId ) ) ? '' : $setId,
															  'css_group'      => $cssName,
					...
					if ( substr( $_name, -4 ) == '.php' )
					{
						...
						foreach( $functions as $fname => $ftext )
						{
							...
							$array = array( 'template_set_id'     => $setId,
											...
											'template_master_key' => ( is_numeric( $setId ) ) ? '' : $setId,
											'template_added_to'   => ( is_numeric( $setId ) ) ? intval( $setId ) : 0 );
	 ...
	 * @return	boolean
	 */
	public function flagSetForRecache( $setId=null )
	{
		/* Update DB row */
		if ( $setId !== null )
		{
	...
	protected function _writeCSSFile( $setID, $css_group, $css_content )
	{
		...
		if ( ! SAFE_MODE_ON )
		{
			...
			if ( is_writeable( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/' ) === TRUE )
			{
				...
				{
					if ( ! @ mkdir( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_' . $setID, 0777 ) )
					{
					...
					else
					{
						...
						@chmod( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_'.$setID, 0777 );
						$good_to_go = 1;
					}
				}
				else
				{
					...
					else if ( file_exists( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_'.$setID.'/'.$css_group.'.css' ) )
					{
		...
		if ( $good_to_go )
		{
			if ( $FH = @fopen( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_'.$setID.'/'.$css_group.'.css', 'w' ) )
			{
				...
				fclose( $FH );
				@chmod( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_'.$setID.'/'.$css_group.'.css', 0777 );
