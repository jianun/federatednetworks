...
class skinFunctions
{
	...
	public function removeSet( $setID )
	{
		...
		/* Delete CSS Files */
		$this->registry->adminFunctions->removeDirectory( DOC_IPS_ROOT_PATH . PUBLIC_DIRECTORY . '/style_css/css_' . $setID );
	...
	public function fetchCSSForEdit( $cssID, $setID )
	{
		...
		$css['_isMaster']		  = ( $css['css_master_key'] ) ? true : false;
...
//		$css['css_content'] = IPSText::textToForm( $css['css_content'] );
	...
	public function fetchTemplateBitForEdit( $templateID, $setID )
	{
		...
		$template['_isMaster']		   = ( $template['template_master_key'] ) ? true : false;
		...
		try
		{
			foreach( new DirectoryIterator( $path ) as $file )
			{
				if ( ! $file->isDot() AND $file->isDir() )
				{
					...
					if( ! file_exists( $path . '/' . $_name . '/conf.php' ) )
					{
						continue;
					}
