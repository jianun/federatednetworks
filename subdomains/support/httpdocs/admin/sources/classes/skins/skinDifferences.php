 ...
 * IP.Board v3.1.2
 * Skin Functions
 * Last Updated: $Date: 2010-05-06 20:30:56 -0400 (Thu, 06 May 2010) $
 * </pre>
 *
 * Owner: Matt
...
class skinDifferences extends skinCaching
{
	 ...
	 */
	public function fetchDifferences( $original, $new )
	{
		$this->classDifference->diff_found = false;
		
		$diff = $this->classDifference->getDifferences( $original, $new );
	...
	public function fetchSession( $diffSessionID, $parse=false )
	{
		...
		if ( $session['merge_id']  AND $parse === true )
		{
			...
			/* Prep dates */
			$session['_date'] = ipsRegistry::getClass( 'class_localization')->getDate( $row['merge_date'], 'TINY' );
