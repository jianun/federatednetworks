...
class twitter_connect
{
	...
	*/
	...
	public function __construct( $registry, $token='', $secret='' )
	{
		...
		/* Set include path.. */
		@set_include_path( IPS_KERNEL_PATH . 'twitter/' . PATH_SEPARATOR . ini_get( 'include_path' ) );
	...
	public function updateStatus( $text )
	{
		if ( $text AND $this->isConnected() )
		{
			...
			if ( $code == 200 AND $status['id'] )
			{
				/* Update member */
				if ( $this->memberData['member_id'] )
				{
					/* Update member */
					IPSMember::save( $this->memberData['member_id'], array( 'extendedProfile' => array( 'tc_last_sid_import' => $status['id'] ) ) );
				}
				...
				if ( $rToken['oauth_token'] AND $rToken['oauth_token_secret'] )
				{
					...
					if ( $connectData['t_key'] )
					{
						...
						else
						{
							...
							/* Generate BW options */
							foreach( array( 'tc_s_pic', 'tc_s_avatar', 'tc_s_status', 'tc_s_aboutme', 'tc_si_status' ) as $field )
							{
								$_toSave[ $field ] = 1;
							}
							...
							$toSave = array( 'core' 		 => array(  'name' 				     => $safeFBName,
											...
											'extendedProfile' => array( 'pp_about_me'            => IPSText::getTextClass( 'bbcode' )->stripBadWords( IPSText::convertCharsets( $_userData['description'], 'utf-8', IPS_DOC_CHAR_SET ) ),
																		'tc_photo'               => $_userData['profile_image_url'],
																		'tc_bwoptions'           => $tc_bwoptions,
																		'avatar_location'        => $_userData['profile_image_url'],
																		'avatar_type'            => 'twitter' ) );
	...
	public function finishConnection()
	{
		...
		$this->registry->getClass('output')->silentRedirect( $this->settings['base_url'] . 'app=core&module=usercp&tab=members&area=twitter' );
	}
	...
	public function syncMember( $memberData )
	{
		...
		try
		{
			...
			if ( $this->isConnected() )
			{
				...
				/* Update.. */
				$exProfile['tc_photo'] = ( $bwOptions['tc_s_pic'] ) ? $user['profile_image_url'] : '';
				
				if ( $bwOptions['tc_s_avatar'] )
				{
					$exProfile['avatar_location'] = $user['profile_image_url'];
					$exProfile['avatar_type']     = 'twitter';
				}
				...
				if ( $bwOptions['tc_si_status'] AND ( isset( $memberData['gbw_no_status_import'] ) AND ! $memberData['gbw_no_status_import'] ) )
				{
					/* Fetch timeline */
				...
				{
					/* remove bg images */
					IPSMember::getFunction()->removeUploadedBackgroundImages( $member['member_id'] );
	...
	public function fetchUserTimeline( $userId=0, $minId=0, $stripReplies=true )
	{
		...
		$updates = $this->_api->get('statuses/user_timeline', array( 'id' => $id, 'count' => 50, 'since_id' => $minId ) );
