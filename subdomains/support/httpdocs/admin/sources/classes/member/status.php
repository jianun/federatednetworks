...
class memberStatus
{
	...
	*/
	 ...
	 * @var		array
	 */
	protected $_internalData		= array( 'IsImport' => 0, 'Creator' => 'ipb', 'ExternalUpdates' => array() );
	...
	*/
    ...
    public function fetchActions( $author=null, $filters=array() )
    {
		...
		while( $row = $this->DB->fetch( $o ) )
		{
			...
			/* Format member */
			$row = IPSMember::buildDisplayData( $row, array( 'reputation' => 0, 'warn' => 0, 'avatar' => 0 ) );
		...
		if ( count( $mids ) )
		{
			...
			foreach( $actions as $id => $row )
			{
				if ( $row['status_member_id'] )
				{
					$actions[ $id ]['owner'] = IPSMember::buildDisplayData( $members[ $row['status_member_id'] ], array( 'reputation' => 0, 'warn' => 0, 'avatar' => 0 ) );
				}
			}
		}
     ...
     * The 'member_id' field will only fetch status updates made by member_id (int)
    ...
    public function fetch( $author=null, $filters=array() )
    {
    	...
    	if ( isset( $filters['friends_only'] ) AND $filters['friends_only'] )
    	{
    	...
    	$offset     = ( isset( $filters['offset'] ) ) ? intval( $filters['offset'] ) : 0;
    	$limit      = ( isset( $filters['limit'] ) ) ? intval( $filters['limit'] ) : 10;
    	$member_id  = ( $filters['member_id'] ) ? $filters['member_id'] : 0;
				...
				if ( is_array( $row['replies'] ) )
				{
					...
					{ 
						if ( $members[ $r['reply_member_id'] ] )
						{
							...
							{
								$members[ $r['reply_member_id'] ] = IPSMember::buildDisplayData( $members[ $r['reply_member_id'] ], array( 'reputation' => 0, 'warn' => 0, 'avatar' => 0 ) );
    ...
    public function fetchAllReplies( $status=null, $filters=array() )
    {
    	...
    	$this->DB->build( array( 'select'   => 's.*',
							     ...
							     'where'    => 's.reply_status_id=' . $status['status_id'],
							     'order'    => 's.' . $sort_field . ' ' . $sort_dir,
							     'limit'    => array( $offset, $limit ),
							     'add_join' => array(array(  'select'	=> 'm.*',
															 'from'	    => array( 'members' => 'm' ),
		...
		while( $row = $this->DB->fetch( $o ) )
		{
			...
			$row['_canDelete']			 = $this->canDeleteReply( $author, $row, $status );
    ...
    public function create( $author=null )
    {
    	...
    	{ 
    		if ( $this->getContent() )
    		{
    			...
    			$test = $this->fetchByHash( $author['member_id'], $hash );
    			...
    			$data = array( 'status_member_id' => $author['member_id'],
							   'status_date'	  => time(),
    			...
    			{
    				$this->_triggerExternalUpdates( $eU, $status_id, $author, $content );
    			}
    			...
    			{
	    			$this->DB->build( array( 'select' => 'friends_member_id', 'from' => 'profile_friends', 'where' => 'friends_friend_id=' . $author['member_id'] ) );
	    			$this->DB->execute();
				...
				if( count($friends) )
				{
	    			...
	    			{	
			    		...
			    		$ndata = array( 'NAME'		=> $friend['members_display_name'],
			    					   'OWNER'		=> $author['members_display_name'],
						...
						IPSText::getTextClass('email')->subject	= sprintf( 
																			...
																			$this->settings['base_url'] . 'app=members&module=profile&section=status&do=list&status_id=' . $status_id
																		);
		
						$notifyLibrary->setMember( $friend );
						$notifyLibrary->setFrom( $author );
						$notifyLibrary->setNotificationKey( 'friend_status_update' );
						$notifyLibrary->setNotificationUrl( $this->settings['base_url'] . 'app=members&module=profile&section=status&do=list&status_id=' . $status_id );
     ...
     * @todo At some point it could be expanded into a mini framework with plugins 
     *
     * @access	protected
    ...
    protected function _triggerExternalUpdates( $updates, $status_id=0, $author=null, $content=null )
    {
    	...
    	if ( $updates['twitter'] )
    	{
    		if ( IPSLib::twitter_enabled() AND $author['twitter_id'] )
    		{
				...
				$twitter = new twitter_connect( $this->registry, $author['twitter_token'], $author['twitter_secret'] );
    ...
    public function fetchMemberLatest( $memberId )
    {
    	...
    	$this->DB->build( array( 'select'   => 's.*',
							     'from'	    => array( 'member_status_updates' => 's' ),
							     'where'    => 's.status_member_id=' . intval( $memberId ),
    ...
    public function creatorImage( $status )
    {
    	...
    	/* Image exists? */
    	if ( ! file_exists( IPS_PUBLIC_PATH . 'style_status/' . $creator . '.png' ) )
    	{
    		$creator = 'ipb';
    	}
	...
	* @return	bool
	*/
	public function canCreate( $author=null )
	{
		$author = ( $author === null ) ? $this->getAuthor() : $author;
