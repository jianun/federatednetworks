 ...
 * Last Updated: $Date: 2010-07-20 05:43:00 -0400 (Tue, 20 Jul 2010) $
 * </pre>
 *
 * @author 		$Author: mark $
...
class output
{
	...
	public function __construct( ipsRegistry $registry, $initialize=FALSE )
	{
		...
		if ( $initialize === TRUE )
		{
			...
			else if ( $this->skin['set_key'] == 'mobile' )
			{
				$this->setAsMobileSkin( true );
			}	
	...
	private function _setHTTPS()
	{
		...
		if( (ipsRegistry::$request['section'] == 'login' OR ipsRegistry::$request['section'] == 'lostpass' OR ipsRegistry::$request['section'] == 'register' or ipsRegistry::$request['module'] == 'usercp') AND $this->settings['logins_over_https'] )
		{
	...
	protected function _fetchUserSkin()
	{
		...
		if ( $this->member->isMobileApp AND is_array( $this->allSkins ) )
		{
		...
		if ( $this->settings['allow_skins'] )
		{
			if ( isset( $this->request['settingNewSkin'] ) AND $this->request['settingNewSkin'] AND ( $this->request['k'] == $this->member->form_hash ) )
			{
	...
	public function buildUrl( $url, $urlBase='public', $httpauth="false" )
	{
		...
		if ( $this->settings['logins_over_https'] )
		{
			if ( 
				...
				( stripos( $url, 'module=usercp' ) !== false AND stripos( $url, 'tab=core' ) !== false AND ( stripos( $url, 'area=password' ) !== false OR stripos( $url, 'area=email' ) !== false OR stripos( $url, 'area=displayname' ) !== false ) )
				)
			{
				$base = str_replace( 'http://', 'https://', $base );
			}
	...
	public function checkPermalink( $seoTitle, $return=FALSE )
	{
		...
		/* Bug Fix: #20279 */
		if( $this->settings['htaccess_mod_rewrite'] && strpos( $_uri, 'index.php?/') )
		{
			$this->registry->getClass('output')->silentRedirect( $this->settings['board_url'] . $_qs, $seoTitle, TRUE, $key );
		}
...
//print "#\d+?" . $_st . preg_quote( $seoTitle, '#' ) . '(' . $_end . "$|" . preg_quote( $_blk, '#' ) . ")#";exit;
	...
	{
		$this->_navigation[] = array( $title, $url, $seoTitle, $seoTemplate, $base );
	}
	 ...
	 *											   'seoTemplate'		=> 'showtopic' ) );								# The FURL template to use </code>
	 *
	 * @access	public
	...
	public function buildRemapData( $FORCE=FALSE )
	{
		...
		if ( ( IN_DEV or $FORCE ) and file_exists( DOC_IPS_ROOT_PATH . 'cache/skin_cache/masterMap.php' ) )
		{
			...
			if ( is_array( $REMAP ) )
			{
				...
				{
					if ( is_array( $REMAP['masterKeys'] ) )
					{
						if ( in_array( $REMAP['inDevDefault'], $REMAP['masterKeys'] ) )
						{
    ...
    public function showError( $message, $code=0, $logError=FALSE, $logExtra='', $header=500 )
    {
    	...
    	$this->outputFormatClass->setHeaderCode( $header ? $header : 500 );
    ...
    public function templateHooks( $text )
    {
		...
		{
			if( is_array( $hooksCache['templateHooks'][$skinGroup] ) AND count( $hooksCache['templateHooks'][$skinGroup] ) )
			{
				foreach( $hooksCache['templateHooks'][$skinGroup] as $tplHook )
				{
					...
					if( file_exists( DOC_IPS_ROOT_PATH . 'hooks/' . $tplHook['filename'] ) )
					{
						...
						if( class_exists( $tplHook['className'] ) )
						{
							...
							{
								$text = $_hook->replaceOutput( $text, $arr_key );
							}
