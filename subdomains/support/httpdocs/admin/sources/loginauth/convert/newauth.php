 ...
 * Application Files
 * Login functions
 * Last Updated: $Date: 2010-04-06 14:26:38 +0100 (Tue, 06 Apr 2010) $
 *
...
class converters
{
	...
	public function go($username, $email, $password)
	{
		...
		foreach ($cycle as $sw)
		{
			if ( $sw == 'vbulletin_legacy36' ) { $sw = 'vulletin_legacy'; }
	 ...
	 */
	private function webwiz( $username, $password )
	{
		$single_md5_pass 	= md5( $password );
		$success 			= false;

		if ( webWizAuth::HashEncode($password . $this->memberData['misc']) == $this->memberData[ $this->password_field ] )
		{
			$success = true;
		}
	 ...
	 private function decrypterChaine($p_strChaine, $p_strClee, $p_strSubst)
	{
		...
		// dŽcryptage, mme mŽthode que le cryptage
	 ...
	 * @return	boolean		Successful
	 */
	private function _webwiz( $username, $password )
	{
		$single_md5_pass 	= md5( $password );
		$success 			= false;
