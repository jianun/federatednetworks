 ...
 * Last Updated: $LastChangedDate: 2010-07-16 17:47:34 -0400 (Fri, 16 Jul 2010) $
 * </pre>
 *
	...
	public function install_sql()
	{
		...
		if ( file_exists( $file ) )
		{
			...
			if ( is_array( $SQL ) AND count( $SQL ) > 0 )
			{
				...
				$message[] = $count . " queries run ({$warnings} non-fatal warnings logged)...";
			}
		}
		else
		{
	...
	public function install_templates()
	{
		...
		else
		{
			...
			foreach( $skinSets as $skinKey => $skinData )
			{
				...
				{
					$skinFunctions->importReplacementsXMLArchive( file_get_contents( IPS_PUBLIC_PATH . 'resources/skins/replacements_' . $skinKey . '.xml' ), $skinKey );
				}
			}
