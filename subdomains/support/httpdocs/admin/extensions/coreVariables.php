 ...
 * Last Updated: $Date: 2010-06-18 06:46:14 -0400 (Fri, 18 Jun 2010) $
 * </pre>
 *
 * @author 		$Author: matt $
...
class coreVariables
{
	...
	public function fetchRedirects()
	{
		...
		# TOPIC
		if( isset( $_REQUEST['showtopic'] ) && $_REQUEST['showtopic'] OR $_REQUEST['act'] == 'ST' )
		{
			$_RESET['app']     = 'forums';
			$_RESET['module']  = 'forums';
		...
		if ( isset( $_REQUEST['act'] ) && strtolower($_REQUEST['act']) == 'paysubs' )
		{
	...
	{
		$_BITWISE = array( 'facebook' => array( 'fbc_s_pic', 'fbc_s_avatar', 'fbc_s_status', 'fbc_s_aboutme', 'fbc_si_status' ), // facebook == profile_portal.fb_bwoptions
						   'twitter'  => array( 'tc_s_pic', 'tc_s_avatar', 'tc_s_status', 'tc_s_aboutme', 'tc_s_bgimg', 'tc_si_status' ), // twitter = profile_portal.tc_bwoptions
						   ...
						   'groups'   => array( 
											    ...
											    'gbw_soft_delete'				, # 1 is ok, 0 is not
											    'gbw_soft_delete_own'			, # 1 is ok, 0 is not
											    'gbw_soft_delete_own_topic'		, # 1 is ok, 0 is not
											    'gbw_un_soft_delete'			, # 1 is ok, 0 is not
											    'gbw_soft_delete_see'			, # 1 is ok, 0 is not
											    'gbw_soft_delete_topic'			, # 1 is ok, 0 is not
											    'gbw_un_soft_delete_topic'		, # 1 is ok, 0 is not
											    'gbw_soft_delete_topic_see'		, # 1 is ok, 0 is not
											    'gbw_soft_delete_reason'		, # 1 means can read reason, 0 means cannot
											    'gbw_soft_delete_see_post'		, # seriously.
											    'gbw_allow_customization'		, # 1 YES, 0 NO
											    'gbw_allow_url_bgimage'			, #^^
											    'gbw_allow_upload_bgimage'		, #^^
											    'gbw_view_reps'					, # 1 allows them to view who gave rep to a post, 0 is can't
											    'gbw_no_status_import'			, # 1 prevents members in group from importing statuses from services.
											  )
						);
