 ...
 * IP.Board v3.1.2
 * Moderator actions
 * Last Updated: $Date: 2010-07-07 08:09:48 -0400 (Wed, 07 Jul 2010) $
 * </pre>
 *
 * @author 		$Author: matt $
...
class moderatorLibrary
{
	...
	public function postToggleSoftDelete( $postIDs, $delete=FALSE, $reason='', $topicIDFix=0 )
	{
		...
		if ( count( $_pids ) )
		{
			$this->DB->update( 'posts', array( 'queued' => $_deletedPost ), 'pid IN (' . implode( ",", $_pids ) . ')' );
	...
	public function rebuildTopic( $tid, $doforum=1, $search_index=1 )
	{
		...
		$posts		= $this->DB->buildAndFetch( array( 'select' => 'COUNT(*) as posts', 'from' => 'posts', 'where' => "topic_id={$tid} and queued=0" ) );
		...
		//-----------------------------------------
		// Get the correct number of soft deleted replies
		//-----------------------------------------
		
		$dposts		= $this->DB->buildAndFetch( array( 'select' => 'COUNT(*) as posts', 'from' => 'posts', 'where' => "topic_id={$tid} and queued=2" ) );
		...
		$last_post	= $this->DB->buildAndFetch( array(
													...
													'limit'		=> array( 1 ),
													'add_join'	=> array(
																		array( 'select'	=> 't.forum_id, t.title',
																				'from'	=> array( 'topics' => 't' ),
																				'where'	=> 't.tid=p.topic_id',
																				'type'	=> 'left'
																			),
		...
		$this->DB->update( 'topics', array( 
											...
											'title_seo'			  => IPSText::makeSeoTitle( $last_post['title'] )
							), 'tid=' . $tid );
	...
	public function topicDelete( $id, $nostats=0 )
	{
		...
		/* Remove from deletion log */
		$_ids = ( is_array( $id ) ) ? $id : array( $id );
		IPSDeleteLog::removeEntries( $_ids, 'topic', TRUE );
		...
		/* Run moderation sync */
		$this->runModSync( 'topicDelete', array_keys( $posts ) );
	...
	public function topicToggleSoftDelete( $topicIds, $delete=FALSE, $reason='' )
	{
		...
		foreach( $_topics as $tid => $topicData )
		{
			...
			/* Mod log */
			$this->addModerateLog( $topicData['forum_id'], $tid, 0, $topicData['title'], sprintf( $this->lang->words['acp_altered_topics'], array(), $topicData['title'] ) );
		}
	...
	public function addModerateLog($fid, $tid, $pid, $t_title, $mod_title='Unknown')
	{
		...
		$this->DB->insert( 'moderator_logs', array (
												  ...
												  'ip_address'		=> $this->member->ip_address,
												  'http_referer'	=> substr( htmlspecialchars(my_getenv('HTTP_REFERER')), 0, 255 ),
												  'ctime'			=> time(),
	...
	public function runModSync( $func )
	{
		...
		foreach( $app_cache as $app_dir => $app )
		{
			...
			$_class = $app['app_directory'] . '_forumDataSync';
