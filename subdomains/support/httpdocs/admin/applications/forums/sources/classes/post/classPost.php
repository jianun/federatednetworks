...
class classPost
{
	 ...
	 * @var		boolean
	 */
	private $_bypassPermChecks = false;
	...
	public function addReply()
	{
		...
		if ( $this->getAuthor('member_id') AND $this->settings['post_merge_conc'] )
		{
			...
			if ( ( $topic['last_post'] > $time_check ) AND ( $topic['last_poster_id'] == $this->getAuthor('member_id') ) )
			{
				...
				if ( $last_post['author_id'] == $this->getAuthor('member_id') )
				{
					...
					$_autoMergeData = array( 'post' => $new_post, 'post_date' => time(), 'pid' => $last_post['pid'] );
		...
		if( $post['post_parent'] )
		{
			$_quoted	= $this->DB->buildAndFetch( array( 'select' => 'author_id', 'from' => 'posts', 'where' => 'pid=' . $post['post_parent'] ) );
			
			//-----------------------------------------
			// Only send if we don't quote ourselves
			// *ahem* Brandon D *ahem*
			//-----------------------------------------
			
			if( $_quoted['author_id'] AND $_quoted['author_id'] != $this->getAuthor('member_id') )
			{
				$_toMember	= IPSMember::load( $_quoted['author_id'] );
				...
				IPSText::getTextClass('email')->buildMessage( array(
																	...
																	'ORIGINAL_POST'	=> $this->registry->output->buildSEOUrl( "showtopic={$post['topic_id']}&amp;view=findpost&amp;p={$post['post_parent']}", "publicNoSession", $topic['title_seo'], 'showtopic' ),
				...
				IPSText::getTextClass('email')->subject	= sprintf( 
																	...
																	$this->registry->output->buildSEOUrl( "showtopic={$post['topic_id']}&amp;view=findpost&amp;p={$post['post_parent']}", "publicNoSession", $topic['title_seo'], 'showtopic' )
																);
	...
	public function editPost()
	{
		...
		if ( $this->_originalPost['new_topic'] == 1 )
		{
			...
			{
				$this->DB->update( 'topics', array( 'icon_id' => intval($post['icon_id']) ), 'tid='.$topic['tid'] );
			}
		}
			...
			if ( is_array( $this->poll_questions ) AND count( $this->poll_questions ) )
			{
				...
				if( $topic['poll_state'] )
				{
					...
					if ( $this->poll_data['choices'] != serialize( $this->poll_questions ) OR $this->poll_data['votes'] != intval($this->poll_total_votes) )
					{
						$this->DB->insert( 'moderator_logs', array( 'forum_id'    => $this->getForumData('id'),
																	...
																	'ctime'       => time(),
																	'topic_title' => IPSText::mbsubstr( $topic['title'], 0, 128 ),
																	'action'      => IPSText::mbsubstr( $this->lang->words['edited_poll'], 0, 128 ),
																	'query_string'=> IPSText::mbsubstr( htmlspecialchars( my_getenv( 'QUERY_STRING' ) ), 0, 128 ) 
										)	);
					}
				}
				else
				{
					...
					$this->DB->insert( 'moderator_logs', array ( 'forum_id'    => $this->getForumData('id'),
																 ...
																 'ctime'       => time(),
																 'topic_title' => IPSText::mbsubstr( $topic['title'], 0, 128 ),
																 'action'      => IPSText::mbsubstr( sprintf( $this->lang->words['added_poll'], $this->request['poll_question'] ), 0, 128 ),
																 'query_string'=> IPSText::mbsubstr( htmlspecialchars(my_getenv('QUERY_STRING')), 0, 128 ) ) );
		...
		if ( $this->edit_title == 1 )
		{
			...
			if ( $this->_topicTitle != "" )
			{
				if ( ($this->_topicTitle != $topic['title']) or ( $this->_topicDescription != $topic['description'] ) OR ! ( $topic['title_seo'] ) )
				{
					...
					/* Data Hook Location */
					IPSLib::doDataHooks( &$_topicTitleData, 'editPostUpdateTopicTitle' );
					
					$this->DB->update( 'topics', $_topicTitleData , "tid=" . $topic['tid'] );
					
					$_forumUpdate	= array();
					
					if ( $topic['tid'] == $this->getForumData('last_id') )
					{
						$_forumUpdate['last_title']		= $_topicTitleData['title'];
						$_forumUpdate['seo_last_title']	= $_topicTitleData['seo_last_title'];
					}
					
					if( $topic['tid'] == $this->getForumData('newest_id') )
					{
						$_forumUpdate['newest_title']		= $_topicTitleData['title'];
					}
					...
					if ( ($this->moderator['edit_topic'] == 1) OR ( $this->getAuthor('g_is_supmod') == 1 ) )
					{
						$this->DB->insert( 'moderator_logs', array(
																	...
																	'ctime'       => time(),
																	'topic_title' => IPSText::mbsubstr( $topic['title'], 0, 128 ),
																	'action'      => IPSText::mbsubstr( sprintf( $this->lang->words['edited_topic_title'], $topic['title'], $this->_topicTitle ), 0, 128 ),
																	'query_string'=> IPSText::mbsubstr( htmlspecialchars(my_getenv('QUERY_STRING')), 0, 128 ),
															)    );
					}
				}
	...
	public function sendOutTrackedTopicEmails( $tid=0, $post="", $poster=0, $last_post=0, $member_id=0 )
	{
		...
		$this->DB->build( array( 
								...
								'add_join' => array(
													array( 
															'select' => 'm.members_display_name, m.email, m.member_id, m.email_full, m.language, m.org_perm_id, m.member_group_id, m.mgroup_others, m.last_activity, m.posts, m.member_banned, m.members_cache, m.ips_mobile_token',
															'from'   => array( 'members' => 'm' ),
															'where'  => 'm.member_id=tr.member_id',
															'type'   => 'left' ),
													array( 
															'select' => 't.title, t.forum_id, t.approved, t.title_seo',
															'from'   => array( 'topics' => 't' ),
		...
		if( $this->DB->getTotalRows($outer) )
		{
			while ( $r = $this->DB->fetch($outer) )
			{
				...
				if ( ! $this->registry->getClass('class_forums')->checkEmailApproved($r, false) )
				{
					continue;
				}
				...
				if ($r['email_full'] == 1)
				{
					...
					IPSText::getTextClass( 'email' )->buildMessage( array(
														...
														'POSTER'          => $poster,
														'POST'            => IPSText::getTextClass( 'bbcode' )->stripBadWords( $post ),
													  )
												);
				...
				IPSText::getTextClass('email')->subject	= sprintf( 
																	IPSText::getTextClass('email')->subject, 
																	$this->registry->output->buildSEOUrl( 'showuser=' . $_poster['member_id'], 'publicNoSession', $_poster['members_seo_name'], 'showuser' ), 
																	$poster,
																	$this->registry->output->buildSEOUrl( 'showtopic=' . $r['topic_id'] . '&amp;view=getnewpost', 'publicNoSession', $r['title_seo'], 'showtopic' ),
																	IPSText::truncate( $r['title'], 50 )
																);
				...
				$notifyLibrary->setNotificationKey( 'new_reply' );
				$notifyLibrary->setNotificationUrl( $this->registry->output->buildSEOUrl( 'showtopic=' . $r['topic_id'] . '&amp;view=getnewpost', 'publicNoSession', $r['title_seo'], 'showtopic' ) );
				$notifyLibrary->setNotificationText( IPSText::getTextClass('email')->message );
				$notifyLibrary->setNotificationTitle( IPSText::getTextClass('email')->subject );
			...
			if( count($_toUpdate) )
			{
				$this->DB->update( "tracker", array( 'last_sent' => time() ), "trid IN(". implode( ',', $_toUpdate ) . ")" );
			}
		}
	...
	public function sendOutTrackedForumEmails( $fid=0, $this_tid=0, $title="", $forum_name="", $post="", $mid=0, $mname="" )
	{
		...
		$mid 	= intval( $mid > 0 ? $mid : $this->getAuthor('member_id') );
		$mname	= $mname != '' ? $mname : $this->getAuthor('members_display_name');
		
		$_poster	= IPSMember::load( $mid );
		...
		$this->DB->build( array( 
								...
								'add_join' => array( 
													array( 
															'select' => 'm.members_display_name, m.member_group_id, m.email, m.member_id, m.language, m.last_activity, m.org_perm_id, m.mgroup_others, m.posts, m.members_cache, m.ips_mobile_token',
															'from'   => array( 'members' => 'm' ),
		...
		{			
			foreach( $gotem as $mid => $r )
			{
				...
				IPSText::getTextClass('email')->subject	= sprintf( 
																	...
																	$this->registry->output->buildSEOUrl( 'showuser=' . $_poster['member_id'], 'publicNoSession', $_poster['members_seo_name'], 'showuser' ), 
																	$mname,
																	$this->registry->output->buildSEOUrl( 'showtopic=' . $this_tid . '&amp;view=getnewpost', 'publicNoSession', IPSText::makeSeoTitle( $title ), 'showtopic' ),
																	IPSText::truncate( $title, 50 )
																);
				...
				$notifyLibrary->setMember( $r );
				$notifyLibrary->setFrom( $_poster );
				$notifyLibrary->setNotificationKey( 'new_topic' );
				$notifyLibrary->setNotificationUrl( $this->registry->output->buildSEOUrl( 'showtopic=' . $this_tid . '&amp;view=getnewpost', 'publicNoSession', IPSText::makeSeoTitle( $title ), 'showtopic' ) );
				$notifyLibrary->setNotificationText( IPSText::getTextClass('email')->message );
				$notifyLibrary->setNotificationTitle( IPSText::getTextClass('email')->subject );
	...
	*/
	...
	{ 
		...
		IPSText::getTextClass( 'editor' )->class_editor->allow_html = (intval($this->request['post_htmlstatus']) AND $this->getForumData('use_html') AND $this->getAuthor('g_dohtml')) ? 1 : 0;
		
		$postContent = IPSText::getTextClass( 'editor' )->processRawPost( $postContent, true );
	...
	*/
	...
	protected function addToModLog( $title='unknown', $topic_title )
	{
		$this->DB->insert( 'moderator_logs', array (
												...
												'ctime'       => time(),
												'topic_title' => IPSText::mbsubstr( $topic_title, 0, 128 ),
												'action'      => IPSText::mbsubstr( $title, 0, 128 ),
												'query_string'=> IPSText::mbsubstr( htmlspecialchars( my_getenv('QUERY_STRING') ), 0, 128 ),
										     ) );
	}
	...
	public function incrementUsersPostCount( $inc=1 )
	{
		...
		$count = $this->registry->class_forums->fetchMemberPpdCount( $this->getAuthor('member_id'), $today );
		...
		if( $this->request['qpid'] )
		{
			...
			if( count( $this->quoted_pids ) )
			{
				...
				while( $tp = $this->DB->fetch() )
				{
					...
					if ( $this->registry->permissions->check( 'read', $this->registry->class_forums->forum_by_id[ $tp['forum_id'] ], $perm_array ) === TRUE )
					{
						...
						if( $tmp_post )
						{
							if ( IPSText::getTextClass( 'editor' )->method == 'rte' )
							{
								$raw_post .= "[quote name='" . IPSText::getTextClass( 'bbcode' )->makeQuoteSafe($tp['author_name']) . "' timestamp='" . $tp['post_date'] . "' post='" . $tp['pid'] . "']<br />{$tmp_post}<br />[/quote]<br /><br /><br />";
							}
							else
							{
								/* Knocks out <br />  */
