...
class admin_forums_statistics_stats extends ipsCommand
{
	...
	*/
	...
	public function spamServiceGraph()
	{
		...
		$graph->addSeries( 'Not Spam', array_values( $notSpamData ) );
		$graph->addSeries( 'Spam', array_values( $isSpamData ) );
	...
	public function result_screen($mode='reg')
	{
		...
		/* Query the stats */
		$this->DB->build( array(
										'select' => "MAX( {$sql_field} ) + {$offset} as result_maxdate, COUNT(*) as result_count, {$this->DB->buildDateFormat( $this->DB->buildFromUnixtime( $sql_field ), $sql_date )} as result_time",
										'from'	 => $sql_table,
