...
class public_forums_moderate_moderate extends ipsCommand
{
	...
	protected function _setAsSpammer()
	{
		...
		if ( ! $member['member_id'] )
		{
			$this->_showError( 'moderate_no_permission', 10311900, 403 );
		}
		...
		{
			$this->_showError( 'moderate_no_permission', 10311901, 403 );
		}
	...
	protected function _doEditMember()
	{
		...
		IPSContentCache::update( $member['member_id'], 'sig', $signatureCache );
	...
	protected function _editMember()
	{
		...
		$this->registry->getClass('output')->setTitle( $this->lang->words['cp_em_title'] . ' - ' . ipsRegistry::$settings['board_name'] );
	 ...
	 */
	protected function _checkAndGetMember()
	{
		...
		{
			$this->_showError( 'mod_only_supermods', 10370, 403 );
		}
		...
		{
			$this->_showError( 'mod_admin_edit', 3032, 403 );
		}
		...
		else
		{
			...
			foreach( $tids as $_tid => $pids )
			{
				...
				if ( $this->fromSearch )
				{
					...
					$this->request['desc']		= $this->lang->words['mod_from_id']." ".$topic['tid'];
				}
				...
				$this->DB->update( 'posts', array( 'topic_id' => $new_topic_id, 'new_topic' => 0, 'post_parent' => 0 ), 'topic_id=' . $_tid . " AND pid IN(" . implode( ",", $pids ). ")" ); 
				...
				$this->DB->update( 'posts', array( 'post_parent' => 0 ), 'post_parent IN (' . implode( ',', $pids ) . ')' );
				...
				if ( $this->trash_inuse )
				{
					$this->_addModeratorLog( $this->lang->words['acp_trashcan_post'] . " '{$topic['title']}'" );
				}
				else
				{
	...
	protected function _multiMergePost()
	{
		...
		else
		{
			...
			if ( !$this->request['selectedpids'] or !$this->request['postdate'] or !$this->request['Post'] )
			{
	...
	protected function _multiDeletePostSplash()
	{
		...
		$canHardDelete = ( $this->memberData['g_is_supmod'] OR $this->moderator['delete_post'] ) ? true : false;
	...
	protected function _closeTopic()
	{
		...
		$this->registry->output->redirectScreen( $this->lang->words['p_closed'], $this->settings['base_url'] . "showforum=".$this->forum['id'] . ( $this->request['_from'] == 'forum' ? '&st=' . intval( $this->request['st'] ) : '' ) );
	}
	...
	protected function _softDeleteTopicToggle( $delete )
	{
		...
		if ( ! $result )
		{
			$this->_showError( 'mod_no_delete_topic', 103119 );
		}
		
		$this->modLibrary->topicToggleSoftDelete( array( $this->topic['tid'] ), $delete, $reason );
		
		$this->registry->output->redirectScreen( $this->lang->words[ $lang ], $this->settings['base_url'] . "showforum=" . $this->forum['id'] . '&st=' . intval( $this->request['st'] ), $this->topic['title_seo'] );
	}
	...
	protected function _topicHistory()
	{
		...
		{
			$this->_showError( 'moderate_no_permission', 103119, 403 );
		}
	...
	protected function _pruneStart( $complete_html="" )
	{
		...
		$this->registry->getClass('output')->addNavigation( $this->forum['name'], "{$this->settings['_base_url']}showforum={$this->forum['id']}", $this->forum['name_seo'], 'showforum' );
		$this->registry->output->addContent( $this->output );
		$this->registry->getClass('output')->sendOutput();
	}
	...
	protected function _multiTopicModify()
	{
		...
		else if ( $this->forum['id'] )
		{
			...
			if ( $this->request['nr'] )
			{
				$this->registry->output->silentRedirect( $this->settings['base_url'] . $url );
			}
			else
			{
				$this->registry->output->redirectScreen( $this->lang->words['cp_redirect_topics'], $this->settings['base_url'] . $url, $this->forum['name_seo']  );
	...
	protected function _multiAlterDeleteSplash()
	{
		...
		$canSoftDelete = $this->registry->getClass('class_forums')->canSoftDeleteTopics( $this->forum['id'], array() );
		$canHardDelete = ( $this->memberData['g_is_supmod'] OR $this->moderator['delete_topic'] ) ? true : false;
	...
	protected function _multiSoftDeleteTopics( $delete=1, $reason='' )
	{
		...
		if ( $delete )
		{
			$result = $this->registry->getClass('class_forums')->canSoftDeleteTopics( $this->forum['id'], array() );
		}
		else
		{
	 ...
	 */
	protected function _genericPermissionCheck( $key='', $error='moderate_no_permission', $checkTrash=0, $return=false )
	{
		$pass = 0;
	 ...
	 * @return	void
	 */
	protected function _takeOutTrash()
	{
