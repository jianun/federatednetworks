...
class public_forums_forums_forums extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		/* Data Hook Location */
		IPSLib::doDataHooks( &$data, 'forumViewData' );
...
//print($cookie_sort);exit;
		...
		if( ( $this->settings['show_user_posted'] == 1 ) and ( $this->memberData['member_id'] ) and ( count($topic_ids) ) and ( $parse_dots ) )
		{
			$this->DB->build( array( 
									...
									'from'   => 'posts',
									'where'  => 'queued=0 AND author_id=' . $this->memberData['member_id'] . ' AND topic_id IN(' . implode( ',', $topic_ids ) . ')',
							)	);
		...
		foreach( $topic_array as $topic )
		{
			...
			{
				if( $this->registry->getClass('IPSAdCode')->getAdCode('ad_code_forum_view_topic_code') )
				{
	...
	private function _generateActiveUserData()
	{
		...
		$this->DB->build( array( 
									'select'	=> 's.member_id, s.member_name, s.member_group, s.id, s.login_type, s.location, s.running_time, s.uagent_type, s.seo_name, s.current_module',
