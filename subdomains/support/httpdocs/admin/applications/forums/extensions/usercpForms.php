...
class usercpForms_forums extends public_core_usercp_manualResolver implements interface_usercp
{
	...
	private function _customEvent_modAnnounceSave( $type='add' )
	{
		...
		//-----------------------------------------
		// _cleanPost() changes \n to <br /> automatically
		// so we need to swap back if not auto parsing br tags
		//-----------------------------------------
		
		if( !$this->request['announce_nlbr_enabled'] )
		{
			$announceContent	= str_replace( "<br />", "\n", str_replace( "\n", '', $announceContent ) );
		}
	...
	private function _customEvent_modAnnounceForm( $type='add', $msg='' )
	{
		...
		if ( $announce['announce_post'] )
		{
			...
			{
				$announce['announce_post'] = IPSText::getTextClass( 'bbcode' )->convertForRTE( nl2br( $announce['announce_post'] ) );
			}
			else
			{
				IPSText::getTextClass( 'bbcode' )->parse_smilies	= 1;
