 ...
 * Last Updated: $Date: 2010-07-13 12:34:38 +0100 (Tue, 13 Jul 2010) $
 * </pre>
 *
 * @author 		$Author: matt $
...
class ppi_recent_topics extends public_portal_portal_portal 
{
	...
	public function recent_topics_discussions_last_x()
	{
		...
		if ( count( $forumIdsOk) )
		{
			...
			$this->DB->build( array( 'select'		=> 't.tid, t.title as topic_title, t.posts, t.start_date as post_date, t.views, t.title_seo, t.starter_name',
											'from'		=> array( 'topics' => 't' ),
			...
			while ( $row = $this->DB->fetch($outer) )
			{
				...
				$row = IPSMember::buildProfilePhoto( $row );
	 			...
	 			if( !$row['member_id'] )
	 			{
					$row['members_display_name']	= $this->settings['guest_name_pre'] . $row['starter_name'] . $this->settings['guest_name_suf'];
				}
