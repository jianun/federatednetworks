 ...
 * Last Updated: $Date: 2010-07-20 21:31:07 +0100 (Tue, 20 Jul 2010) $
 * </pre>
 *
...
class admin_members_members_tools extends ipsCommand
{
	...
	public function rebuildStats()
	{
		...
		foreach( $this->caches['group_cache'] as $_gid => $_gdata )
		{
			if( !$_gdata['g_hide_online_list'] )
			{
				$_groups[]	= $_gid;
			}
		}
		
		$_where	= count($_groups) ? " AND member_group_id IN(" . implode( ',', $_groups ) . ")" : "";
		
		$r = $this->DB->buildAndFetch( array( 'select' => 'member_id, name, members_display_name, members_seo_name',
										  'from'   => 'members',
										  'where'  => "member_group_id <> '{$this->settings['auth_group']}'{$_where} AND members_display_name != '' AND members_display_name " . $this->DB->buildIsNull( false ),
										  'order'  => "member_id DESC",
										  'limit'  => array(0,1) ) );

		$stats['last_mem_name_seo']	= $r['members_seo_name'];
	...
	private function _viewQueue( $type='validating' )
	{
		...
		switch( $type )
		{
			...
			case 'banned':
				if( !$this->settings['banned_group'] )
				{
					$this->registry->output->html .= $this->html->bannedGroupNotice();
				}
				
				$row = $this->DB->buildAndFetch( array( 'select'		=> 'COUNT(*) as queue',
																'from'		=> 'members',
																'where'		=> "member_banned=1 OR member_group_id=" . intval($this->settings['banned_group']),
														)		);
			break;
			case 'spam':
				$row	= $this->DB->buildAndFetch( array( 'select'	=> 'COUNT(*) as queue',
				...
				// Also get validating users marked as spam users
				$row2	= $this->DB->buildAndFetch( array( 'select'	=> 'COUNT(*) as queue2',
														'from'		=> 'validating',
														'where'		=> "spam_flag=1" ) );

				$row	= array( 'queue' => $row['queue'] + $row2['queue2'] );
			break;
			case 'incomplete':
				/* Show global message */
				$this->registry->output->global_message = $this->lang->words['inc_glb_msg'];
				
				/* Query */
				$this->DB->build( array( 'select'		=> 'COUNT(*) as queue',
										 'from'		    => array( 'members_partial' => 'p' ),
										 'where'		=> 'm.member_id ' . $this->DB->buildIsNull(false),
										 'group'		=> 'm.member_id',
										 'add_join'     => array( array( 'select' => 'm.member_id',
		...
		if ( $cnt > 0 )
		{
			switch( $type )
			{
				...
				case 'banned':
					$this->DB->build( array( 'select' 	=> 'm.member_group_id, m.members_display_name, m.ip_address, m.member_id, m.email, m.posts, m.joined, m.failed_logins, m.failed_login_count',
													 'from' 	=> array( 'members' => 'm' ),
													 'where' 	=> "m.member_banned=1 OR m.member_group_id=" . intval($this->settings['banned_group']),
													 'order'	=> $col . ' ' . $ord,
													 'limit'	=> array( $st, 75 ),
													 'add_join'	=> array(
													 					array(
				...
				case 'spam':
					$this->DB->build( array( 'select' 	=> 'm.*',
											 'from' 	=> array( 'members' => 'm' ),
											 'where' 	=> IPSBWOptions::sql( 'bw_is_spammer', 'm.members_bitoptions', 'members', 'global', 'has' ) . ' OR v.spam_flag=1',
											 'order'	=> $col . ' ' . $ord,
											 'limit'	=> array( $st, 75 ),
											 'add_join'	=> array(
											 ...
											 'from'		    => array( 'members_partial' => 'p' ),
										 	 ...
										 	 'add_join'     => array( array( 'select' => 'm.*',
										 							 ...
										 							 array( 'select'  => 'pp.*',
										 								     ...
										 								     'where'  => 'pp.pp_member_id=p.partial_member_id' ) ) ) );
			}
			...
			while ( $r = $this->DB->fetch( $o ) )
			{
				...
				{
					case 'incomplete':
						/* Try and determine what app they came in from */
						if ( $r['fb_uid'] )
						{
							$r['_regVia'] = 'facebook';
						}
						else if ( $r['twitter_id'] )
						{
							$r['_regVia'] = 'twitter';
						}
						
						/* Do they have a real email address? */
						$r['_emailIsOk'] = ( $r['email'] == $r['name'] . '@' . $r['joined'] ) ? false : true;
						$r['_dnameIsOk'] = ( $r['members_l_display_name'] )                   ? true : false;
						
						$r['member_id']  = ( $r['partial_member_id'] ) ? $r['partial_member_id'] : $r['member_id'];
		...
		switch( $type )
		{
			...
			case 'spam':
				$this->registry->output->extra_nav[] 		= array( '', $this->lang->words['t_spammers'] );
			break;
			
			case 'incomplete':
				$this->registry->output->extra_nav[] 		= array( '', $this->lang->words['t_incomplete'] );
			break;
		}
	}
	 ...
	 * @return	void		[Outputs to screen]
	 */
	private function _doIncomplete()
	{
		//-----------------------------------------
		// INIT
		//-----------------------------------------
		...
		if ( $this->request['type'] == 'delete' )
		{
			...
			ipsRegistry::getClass('adminFunctions')->saveAdminLog( count($ids) . $this->lang->words['t_inc_removed'] );

			$this->registry->output->global_message = count($ids) . $this->lang->words['t_inc_removed'];
			$this->_viewQueue( 'incomplete' );
			return;
		}
	}
	...
	private function _manageValidating()
	{
		...
		if ( $this->request['type'] == 'approve' )
		{
			...
			while( $row = $this->DB->fetch( $main ) )
			{
				...
				// @see http://forums.invisionpower.com/index.php?autocom=tracker&showissue=11564&view=findpost&p=45269
				//-----------------------------------------
	...
	private function _unban()
	{
		...
		if ( $this->request['type'] == 'unban' )
		{
			...
			try
			{
				foreach( $members as $id => $data )
				{
					if( $data['member_group_id'] == $this->settings['banned_group'] )
					{
	...
	private function _emailUnapprove()
	{
		...
		$this->registry->output->global_message = sprintf( $this->lang->words['t_emailchangeun'], $member['member_id'], $member['temp_group'] );
		$this->_viewQueue( 'validating' );
	}
