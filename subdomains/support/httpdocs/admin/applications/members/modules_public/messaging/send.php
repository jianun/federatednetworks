 ...
 * IP.Board v3.1.2
 * Online list
 * Last Updated: $Date: 2010-07-08 18:54:42 -0400 (Thu, 08 Jul 2010) $
 * </pre>
 *
...
class public_members_messaging_send extends ipsCommand
{
	...
	private function _showForm( $type, $errors='' )
	{
		...
		if ( $msgID AND ! $_POST['Post-NS'] )
		{
			...
			if ( $msgData['msg_post'] )
			{
				if ( $type == 'reply' )
				{
					...
					{
						$displayData['message'] = "[quote name='".IPSText::getTextClass( 'bbcode' )->makeQuoteSafe($msgData['members_display_name'])."' timestamp='" . $msgData['msg_date'] . "']<br />{$msgData['msg_post']}".'[/quote]'."<br /><br /><br />";
					}
					else
					{
    	...
    	if ( $errors OR ( IPSText::getTextClass('bbcode')->error != "" ) )
    	{
 		...
 		{ 
 			// Not needed as is handled in above $type == 'edit' block
 		}
	...
	private function _showNewTopicForm( $errors='' )
	{
		...
		else if( IPSText::getTextClass('editor')->method == 'std' AND $displayData['message'] AND $fromMsgID )
		{
 		...
 		//-----------------------------------------
 		// Is this RTE? If so, convert BBCode
 		// Changed to else if 4.12.2010.  Otherwise, first if above
 		// would parse, then it would parse a second time here.
 		//-----------------------------------------
 		
 		else if ( IPSText::getTextClass('editor')->method == 'rte' AND $displayData['message'] AND !$fromMsgID )
 		{
			if ( count( $errors ) or $preview )
			{
				$displayData['message'] = stripslashes( $displayData['message'] );
