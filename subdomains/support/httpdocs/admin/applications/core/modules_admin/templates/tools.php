...
class admin_core_templates_tools extends ipsCommand
{
	...
	*/
	 ...
	 * @return	string		HTML
	 */
	private function _inDevMasterCSS( $setId=false )
	{
		/* This is which skins we want to export for default installations */
	...
	private function _recache()
	{
		...
		if ( $type == 'all' )
		{
			...
			else
			{
				...
				$this->registry->output->redirect( $this->settings['base_url'] . 'app=core&module=templates&section=tools&do=toolsRecache&type=all&setID=-1', $this->lang->words['to_recachedset'] . $this->registry->output->allSkins[ $setID ]['set_name'], 2, TRUE );
			}
		}
		else
		{
