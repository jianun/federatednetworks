 ...
 * Last Updated: $Date: 2010-05-24 20:18:57 -0400 (Mon, 24 May 2010) $
 * </pre>
 *
...
class admin_core_tools_login extends ipsCommand
{
	...
	private function _loginDiagnostics()
	{
		...
		if( file_exists( IPS_PATH_CUSTOM_LOGIN . '/' . $login['login_folder_name'] . '/auth.php' ) )
		{
			$classFile 	= IPS_PATH_CUSTOM_LOGIN . '/' . $login['login_folder_name'] . '/auth.php';
			$config		= IPS_PATH_CUSTOM_LOGIN . '/' . $login['login_folder_name'] . '/conf.php';
			...
			$classname	= "login_" . $login['login_folder_name'];
        
			require_once( $classFile );
			$module = new $classname( $this->registry, $login, $LOGIN_CONF );
			
			if( $module->missingModules )
			{
	...
	private function _loginList()
	{
		...
		if ( $dh !== false )
		{
			while ( false !== ($file = readdir($dh) ) )
			{
				if( is_dir( IPS_PATH_CUSTOM_LOGIN . '/' . $file ) AND !in_array( $file, array( '.', '..', '.svn', '.DS_Store' ) ) )
				{
					...
					if( file_exists( IPS_PATH_CUSTOM_LOGIN . '/' . $file . '/acp.php' ) )
					{
						if ( file_exists( IPS_PATH_CUSTOM_LOGIN . '/' . $file . '/conf.php' ) )
						{
							$data['acp_plugin']	= 1;
						}
						else if ( file_exists( IPS_PATH_CUSTOM_LOGIN . '/' . $file . '/conf.dist.php' ) )
						{
							$missing[] = IPS_PATH_CUSTOM_LOGIN . '/' . $file . '/';
						}
					}
		...
		//-----------------------------------------
		// Got some folders with the .dist.php files?
		//-----------------------------------------
		
		if ( count( $missing ) )
		{
			$this->registry->output->showError( sprintf( $this->lang->words['l_rename_conf'], implode( "</li><li>", $missing ) ) );
		}
