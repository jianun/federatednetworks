 ...
 * IP.Board v3.1.2
 * ACP : Settings
 * Last Updated: $Date: 2010-07-01 11:37:41 -0400 (Thu, 01 Jul 2010) $
 *
 * @author 		$Author: matt $
...
class admin_core_tools_settings extends ipsCommand
{
	...
	public function importAllApps()
	{
		...
		if ( IN_DEV )
		{
			$notThese = $this->_loadDeleteList();
			
			if ( count( $notThese ) )
			{
				$this->DB->delete( 'core_sys_conf_settings', 'conf_key IN(\'' . preg_replace( "#\s#", "", implode( "','", $notThese ) ) . "')" );
			}
		}
	...
	private function _importXML( $content, $app='core', $knownSettings=array() )
	{
		...
		if ( IN_DEV )
		{
			...
			if ( count( $notThese ) )
			{
				$where = ' AND conf_key NOT IN(\'' . preg_replace( "#\s#", "", implode( "','", $notThese ) ) . "')";
			}
		}
	...
	/**
	 * Load and parse delete list (IN_DEV)
	 *
	 * @access	private
	 * @return	array
	 */
	private function _loadDeleteList()
	{
		if ( IN_DEV )
		{
			...
			{
				return file( DOC_IPS_ROOT_PATH . '_deleteSettings.txt' );
			}
		}
