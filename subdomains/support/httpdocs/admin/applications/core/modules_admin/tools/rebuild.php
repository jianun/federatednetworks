...
class admin_core_tools_rebuild  extends ipsCommand
{
	...
	public function tools300pms()
	{
		...
		if ( $seen )
		{
			...
			$this->registry->output->global_message = sprintf( $this->lang->words['tools300pms'], $this->request['st'] );
	...
	public function doCount()
	{
		...
		$this->registry->output->doneScreen($this->lang->words['re_statsrecount'], $this->lang->words['re_statscenter'], "{$this->form_code}", 'redirect' );
	}
	...
	public function cleanAvatars()
	{
		...
		if ( $i < $dis)
		{
			...
			$text = $this->lang->words['re_rebuildcomplete'] . '<br />' . implode( "<br />", $output );
			$url  = "{$this->settings['base_url']}{$this->form_code}";
			$time = 2;
		}
		else
	...
	public function cleanAttachments()
	{
		...
		if ( $i < $dis)
		{
			...
			$text = $this->lang->words['re_rebuidcomp'] . '<br />' . implode( "<br />", $output );
			$url  = "{$this->settings['base_url']}{$this->form_code}";
			$time = 2;
		}
		else
