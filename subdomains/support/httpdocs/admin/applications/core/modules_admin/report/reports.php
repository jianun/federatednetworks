...
class admin_core_report_reports extends ipsCommand 
{
	...
	private function _createPlugin()
	{
		...
		if( $_POST['finish'] == 1 )
		{
			...
			elseif( ! file_exists( IPSLib::getAppDir( $this->request['appPlugin'] ) . '/extensions/reportPlugins/' . $this->request['plugi_file'] . '.php' ) )
			{
	...
	private function _editPlugin()
	{
		...
		require_once( IPSLib::getAppDir( $plug_data['app'] ) . '/extensions/reportPlugins/' . $plug_data['my_class'] . '.php' );
		...
		if( $_POST['finish'] == 1 )
		{
			...
			$plugin_error = $plugin->processAdminForm( $extra_data );
			
			if( $plugin_error )
			{
				$this->registry->output->global_message = $plugin_error;
			}
	...
	private function _reportsChart()
	{
		...
		$this->DB->build(
								array(
									...
									'where'     => "( stat.is_active=1 OR ( stat.is_active=0 AND (" . time() . "-rep.date_updated)<86400 ) )",
									'group'		=> 'rep.status',
									'add_join'	=> array(
														array(
