...
class admin_core_applications_hooks extends ipsCommand
{
	...
	public function installHook( $content, $addMessage=FALSE, $allowSkinRecache=TRUE, $enabled=1 )
	{
		...
		foreach( $xml->fetchElements('hookextras_css') as $node )
		{
			foreach( $xml->fetchElements('css', $node) as $_css )
			{
				...
				$css[]	= array(
									...
									'css_master_key'	=> ( $_css['css_master_key'] ) ? $_css['css_master_key'] : $_css['css_master_key']
									);
			}
		}
		...
		if( count($templates) )
		{
			...
			$this->DB->build( array( 'select' => 'template_name,template_group', 'from' => 'skin_templates', 'where' => 'template_set_id=0 AND template_master_key=\'root\'' ) );
			$this->DB->execute();
		...
		if( count( $css ) )
		{
			...
			$this->DB->build( array( 'select' => '*', 'from' => 'skin_css', 'where' => 'css_set_id=0' ) );
			$this->DB->execute();
			...
			{
				if( is_array( $bits[ $entry['css_group'] ] ) AND in_array( $entry['css_group'], $bits[ $entry['css_group'] ] ) )
				{
					$this->DB->update( 'skin_css', $entry, "css_group='{$entry['css_group']}'" );
					$cssUpdated++;
				}
				else
				{	
					...
					$cssInserted++;
					$css_id = $this->DB->getInsertId();
					$extra_data['css'][] = $css_id;
				}
			}
		}
...
EOF;
	...
	private function _doExportHook()
	{
		...
		{
			$templateSetIds = $this->registry->output->buildRemapData( true );

			foreach( $extra_data['templates'] as $file => $templates )
			{
				$this->DB->build( array( 'select' => '*', 'from' => 'skin_templates', 'where' => "template_set_id IN ('".implode( "','", $templateSetIds['export']) ."') AND template_group='{$file}' AND template_name IN('" . implode( "','", $templates ) . "')" ) );
				$this->DB->execute();
				
				while( $r = $this->DB->fetch() )
				{
					unset($r['template_id']);
					$r['template_set_type'] = $templateSetIds['templates'][$r['template_set_id']];
		...
		{
			$this->DB->build( array( 'select' => '*', 'from' => 'skin_css', 'where' => "css_id IN(" . implode( ",", $extra_data['css'] ) . ")" ) );
			$this->DB->execute();
	...
	private function _fixPositions()
	{
		...
		while ( $hook = $this->DB->fetch( $qid ) )
		{
			...
			$this->DB->build( array( 'select' => '*', 'from' => 'core_hooks_files', 'where' => "(hook_type='commandHooks' OR hook_type='skinHooks' OR hook_type='libraryHooks') AND hook_hook_id=" . $hook['hook_id'] ) );
			$this->DB->execute();
	...
	private function _hooksOverview()
	{
		...
		$min_order			= 999999;
		$max_order			= 0;
		$checkUpdates		= ( isset( $this->request['checkUpdates'] ) AND $this->request['checkUpdates'] ) ? true : false;
	 ...
	 * @return	void		[Outputs to screen]
	 */
	private function _disableAllHooks()
	{
		/* INIT */
		$activeHookIds = array();
	...
	private function _reenableAllHooks()
	{
		/* INIT */
	...
	private function _disableHook()
	{
		...
		/* Done */
		$this->registry->output->global_message = $this->lang->words['hook_disabled'];
		$this->registry->output->silentRedirectWithMessage( $this->settings['base_url'] . $this->form_code . '&amp;do=hooks_overview' );
	}	
	...
	private function _uninstallHook()
	{
		...
		{
			$this->DB->delete( 'skin_css', 'css_id IN(' . implode( ',', $extra_data['css'] ) . ')' );
		...
		{
			if( file_exists( IPS_HOOKS_PATH . 'install_' . $extra_data['display']['custom'] ) )
			{
				require_once( IPS_HOOKS_PATH . 'install_' . $extra_data['display']['custom'] );
	...
	public function rebuildHooksCache()
	{
		...
		while( $r = $this->DB->fetch() )
		{
			...
			if( $r['hook_type'] == 'templateHooks' )
			{
				...
				//$_cache['skinGroup']		= $data['skinGroup'];
