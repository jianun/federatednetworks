 ...
 * Last Updated: $Date: 2010-05-13 18:14:37 -0400 (Thu, 13 May 2010) $
 * </pre>
 *
...
class admin_core_applications_setup extends ipsCommand
{
	...
	public function recacheTemplates()
	{
		...
		if( $this->request['_recache_done'] == 'true' )
		{
		...
		{
			$this->showRedirectScreen( $vars['app_directory'], array( $this->lang->words['to_recachedset'] . $this->registry->output->allSkins[ $setID ]['set_name'] ), '', $this->getNextURL( 'tplcache&amp;_recache_done=true', $vars ) );			
		}
	...
	public function finish()
	{
		...
		/* Rebuild FURLs */
		if( file_exists( $this->app_full_path . 'extensions/furlTemplates.php' ) )
		{
			try
			{
	...
	public function templates()
	{
		...
		foreach( $skinSets as $skinKey => $skinData )
		{
			...
			if ( file_exists( $this->app_full_path . 'xml/' . $vars['app_directory'] . '_replacements.xml' ) )
			{
				...
				$return = $skinFunctions->importReplacementsXMLArchive( file_get_contents( $this->app_full_path . 'xml/' . $vars['app_directory'] . '_replacements.xml' ) );
