 ...
 * IP.Board v3.1.2
 * Group management
 * Last Updated: $Date: 2010-06-29 22:16:04 -0400 (Tue, 29 Jun 2010) $
 * </pre>
 *
...
class admin_core_applications_applications extends ipsCommand
{
	...
	private function applicationSave( $type='add' )
	{
		...
		if( in_array( $array['app_directory'], array( 'core', 'forums', 'members' ) ) AND !$array['app_enabled'] )
		{
	...
	private function applicationManagePosition()
	{
		...
		if( $this->registry->adminFunctions->checkSecurityKey( $this->request['md5check'], true ) === false )
		{
 		...
 		if( is_array($this->request['apps']) AND count($this->request['apps']) )
 		{
 			...
 			{
 				$location = $this->DB->buildAndFetch( array( 'select' => 'app_location', 'from' => 'core_applications', 'where' => 'app_id=' . $this_id ) );
				$buffer   = array( 'root' => 0, 'ips' => 100, 'other' => 200 );
 				$this->DB->update( 'core_applications', array( 'app_position' => $position+$buffer[ $location['app_location'] ] ), 'app_id=' . $this_id );
	...
	public function applicationsRecache()
	{
		...
		while( $row = $this->DB->fetch($outer) )
		{
			...
			/* Search */
			if( IPSSearchIndex::appIsSearchable( $row['app_directory'] ) )
			{
			...
			/* incomingEmails? */
			if ( file_exists( IPSLib::getAppDir( $row['app_directory'] ) . '/extensions/incomingEmails.php' ) )
			{
				$incomingEmails = TRUE;
			}
		}
		...
		foreach ( $apps as $dir => $data )
		{
			...
			if ( file_exists( IPSLib::getAppDir( $dir ) . '/extensions/coreVariables.php' ) )
			{
