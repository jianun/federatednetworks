 ...
 * IP.Board v3.1.2
 * Attachments
 * Last Updated: $Date: 2010-07-09 23:59:26 -0400 (Fri, 09 Jul 2010) $
 * </pre>
 *
 * <code>
...
class class_attach
{
			...
			foreach( $rows as $_attach_id => $row )
			{
				...
				if ( $this->settings['show_img_upload'] and $row['attach_is_image'] )
				{
					if ( $this->attach_settings['siu_thumb'] AND $row['attach_thumb_location'] AND $row['attach_thumb_width'] )
					{
						...
						$row['_attach_id'] = $row['attach_id'] . '-' . str_replace( ".", "-", microtime() );
	...
	public function processUpload()
	{
		...
		$upload->out_file_name    = $this->type . '-' . $this->memberData['member_id'] . '-' . str_replace( '.', '', microtime() );
	...
	public function loadAttachmentPlugin()
	{
		...
		{
			if( file_exists( IPSLib::getAppDir( $app['app_directory'] ) . '/extensions/attachments/plugin_' . $this->type . '.php' ) )
			{
				$file = IPSLib::getAppDir( $app['app_directory'] ) . '/extensions/attachments/plugin_' . $this->type . '.php';
				break;
			}
		}
