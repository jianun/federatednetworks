 ...
 * Last Updated: $Date: 2010-07-06 23:05:34 +0100 (Tue, 06 Jul 2010) $
 * </pre>
 *
...
<div style='background-color: #fff; overflow: auto; padding: 10px;'>
	{$difference}
</div>
...
<div class='acp-box'>
	...
	<div class='alternate_rows double_pad' id='tplate_groupList'></div>
EOF;
...
<div style='display:none'>
	<div id='tplate_groupRow'>
		<div id='groupRow_#{groupName}' class='#{classname}' style='padding:8px';>
	...
	<div id='tplate_templateRow'>
		<div id='tplate_templaterow_#{template_id}' style='padding:10px 0px 8px 8px;margin-left:20px'>
			...
			<img src='{$this->settings['skin_acp_url']}/_newimages/icons/template.png' />
	...
	<div id='tplate_templateEditor'>
		<div id='tplate_editor_#{template_id}' class='tableborder' style='padding:0px;margin:0px;width:800px'>
			<div id='inlineFormInnerTitle' style='padding:0px'><h4>Editing "#{template_name}" in "#{template_group}"</h4></div>
			<div id='inlineFormInnerContent' style='padding:10px'>
	...
	if ( count( $reports ) )
	{
		foreach( $reports as $group => $key )
		{
			foreach( $reports[ $group ] as $key => $report )
			{
				...
				$IPBHTML .= <<<EOF
					<div style='padding:4px;border:1px solid #000;background-color:#FFF;margin:4px;'>
						<h2>{$prefix}{$report['change_data_group']} <span style='color:green'>&gt;</span> {$report['change_data_title']}</h2>
						<hr>
						{$report['change_data_content']}
					</div>
EOF;
...
<div class='section_title'>
	...
	<ul class='context_menu'>
		<li>
			<a href='#' class='ipbmenu' id='mOptions'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/cog.png' /> {$this->lang->words['mergerepoptions']} <img src='{$this->settings['skin_acp_url']}/_newimages/useropts_arrow.png' /></a>
		</li>		
	</ul>
</div>

<ul class='ipbmenu_content' id='mOptions_menucontent' style='display: none'>
	<li><input type="checkbox" class='__cbx' id='_cb_nocustom' /> <img src='{$this->settings['skin_acp_url']}/_newimages/icons/page_white_magnify.png' /> {$this->lang->words['merge__showall']}</li>
	<li><input type="checkbox" class='__cbx' id='_cb_merged' /> <img src='{$this->settings['skin_acp_url']}/_newimages/icons/shape_move_front.png' /> {$this->lang->words['merge__showmerged']}</li>
	<li><input type="checkbox" class='__cbx' id='_cb_conflicted' /> <img src='{$this->settings['skin_acp_url']}/_newimages/icons/exclamation.png' /> {$this->lang->words['merge__showconflict']}</li>
	<li><input type="checkbox" class='__cbx' id='_cb_uncommitted' /> <img src='{$this->settings['skin_acp_url']}/_newimages/icons/bullet_red.png' /> {$this->lang->words['merge__showcommit']}</li>
	<li><input type="checkbox" class='__cbx' id='_cb_committed' /> <img src='{$this->settings['skin_acp_url']}/_newimages/icons/bullet_green.png' /> {$this->lang->words['merge__showcommita']}</li>
</ul>
...
<div class='acp-box'>
	<div style="float:right;padding-right:32px;padding-top:8px"><input type="checkbox" id="cboxall" /></div>
	<h3>{$sessionData['_title']}</h3>
	<table class='alternate_rows pad'>
...
EOF;
		...
		foreach( $reports[ $group ] as $key => $report )
		{
			...
			$_mergeIcon  = ( $report['_is'] == 'new' OR ! $report['change_can_merge'] ) ? '../../blank.gif' : ( ( intval( $report['_conflicts'] ) > 0 ) ? 'exclamation.png' : '' );//'shape_move_front.png' );
			...
			$IPBHTML .= <<<EOF
				<tr class='__diffRow __rowId{$report['change_id']} __xx{$report['_is']} {$_mergeClass} {$_comClass} _xgid{$_group} {$_mainClass} {$_isMergeClass}'>
				<td width='1%'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$_icon}' /></td>
				 <td width='84%'>
				   <img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$_comIcon}' /> <strong>{$report['change_data_title']}</strong>
				   <div class='desctext' style='font-size:0.8em' id='mDesc-{$report['change_id']}'>{$_desc}</div>
				 </td>
				 <td width='1%' nowrap='nowrap' align='center'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$_mergeIcon}' title='{$_mergeTitle}' id='mMergeImage-{$report['change_id']}' class='__whenHasMerge{$report['change_id']}' /></td>
				 <td width='1%' nowrap='nowrap' align='center'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$_diffIcon}' title='{$_diffTitle}' /></td>
				 ...
				 <td width='1%'>
					...
					<ul class='acp-menu' id='menu{$_safe}_menucontent'>
						...
						<li class='icon view'><a href='javascript:void(0);' onclick="return IPB3TemplateDiffResults.viewDiff({$report['change_id']}, 'diff')">{$this->lang->words['sk_viewdiffs']} {$this->lang->words['menu__oldnew']}...</a></li>
EOF;
			if ( intval( $report['_conflicts'] ) > 0 )
			{
				$IPBHTML .= <<<EOF
...
if ( count( $sessions ) )
{
	foreach( $sessions as $id => $data )
	{
		...
		<tr>
		 ...
		 <td width='5%'>
		 	<img class='ipbmenu' id="menu{$data['merge_id']}" src='{$this->settings['skin_acp_url']}/_newimages/menu_open.png' alt='{$this->lang->words['a_options']}' />
			<ul class='acp-menu' id='menu{$data['merge_id']}_menucontent'>
				<li class='icon view'><a href='{$this->settings['base_url']}&amp;{$this->form_code}&amp;do=viewReport&amp;sessionID={$data['merge_id']}'>{$this->lang->words['sk_viewdiffresults']}...</a></li>
				<!--<li class='icon delete'><a href='#' onclick='return acp.confirmDelete("{$this->settings['base_url']}{$this->form_code}&amp;do=removeReport&amp;sessionID={$data['merge_id']}");'>{$this->lang->words['sk_removediffresults']}...</a></li>-->
				<li class='icon export'><a href='{$this->settings['base_url']}&amp;{$this->form_code}&amp;do=exportReport&amp;sessionID={$data['merge_id']}'>{$this->lang->words['sk_createhtmlexport']}...</a></li>
			</ul>
		 </td>
		</tr>
EOF;
...
<div class='acp-box'>
	<h3>$title</h3>
	<table width='100%' class='form_table alternate_rows'>
		<tr>
 ...
 * @return	string		HTML
 */
public function skinsets_listSkinSets( $sets, $cacheData, $hasData, $canMerge ) {
...
<div class='acp-box' id='forum_wrapper'>
	...
	<table style='width: 100%' cellspacing='0' class='double_pad'>
		<tr>
			<th width='3px'>&nbsp;</th>
			<th style='width: 81%'>{$this->lang->words['sk_setname']}</th>
			<th style='width: 3%'>&nbsp;</th>
			<th style='width: 12%; text-align: center;'>{$this->lang->words['sk_outputformat']}</th>
			<th style='width: 3%'>&nbsp;</th>
		</tr>
	</table>
	<ul class='sortable_handle' id='sort_root'>
EOF;
	...
	{	
		...
		<ul class='sortable_handle' id='sort_{$data['set_id']}'>
		<table style='width: 100%' cellspacing='0' class='double_pad'>
		<tr>
			<td class='{$data['_cssClass']} forum_row {$subskin}' width='1%' style='text-align: center'>
				<div class='draghandle'><img src='{$this->settings['skin_acp_url']}/_newimages/drag_icon.png' alt='' /></div>
			</td>
			 <td class='{$data['_cssClass']} forum_row {$subskin}' style='width: 81%'>
				{$data['depthguide']}
...
EOF;
		...
		$IPBHTML .= <<<EOF
			...
			<td class='{$data['_cssClass']} forum_row {$subskin}' style='text-align: center' style='width: 3%'>
				$hiddenImage
			</td>
			<td class='{$data['_cssClass']} forum_row {$subskin}' style='text-align: center' style='width: 12%'>
				$formatImage
			</td>
			<td class='{$data['_cssClass']} forum_row {$subskin}' style='width: 3%'>
...
EOF;
			...
			{		
			$IPBHTML .= <<<EOF
					<li class='icon edit'><a href='{$this->settings['base_url']}&amp;module=templates&amp;section=skindiff&amp;do=skinDiffStart&amp;setID={$data['set_id']}'>{$this->lang->words['runmergereports']}</a></li>
EOF;
			}
...
<script type='text/javascript'>
	...
	ipb.templates['add_replacement'] = "<div class='acp-box'><h3>{$this->lang->words['add_repl_f_tpl']}</h3><ul class='acp-form'><li><label for='popup_key'>{$this->lang->words['rep_key_f_tpl']}</label><input type='text' class='input_text' id='popup_key' /></li><li><label for'popup_content'>{$this->lang->words['rep_ctn_f_tpl']}</label><textarea id='popup_content' style='width: 45%' rows='7' class='input_text'></textarea></li></ul><div class='acp-actionbar'><input type='submit' class='realbutton' value='{$this->lang->words['addrep_btn_f_tpl']}' id='popup_submit' /></div></div>";
...
EOF;
				...
				<tr>
					<td>
						<strong>{$this->lang->words['sk_upgrade']}</strong><br />
