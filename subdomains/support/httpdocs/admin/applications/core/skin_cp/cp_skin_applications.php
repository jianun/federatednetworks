...
$IPBHTML .= <<<EOF
<img src='{$this->settings['skin_acp_url']}/images/spacer.gif' width='12' height='12' style='vertical-align:middle' />
EOF;
...
EOF;
					...
					<td style='width: 10%'>
						<img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$module['_sys_module_visible']}' />
					</td>
					<td style='width: 3%'>
...
EOF;
	...
	if( !IPSLib::appIsInstalled( 'blog', false ) )
	{
		$IPBHTML .= <<<EOF
<div class='menulinkwrap'>
	<img src='{$this->settings['skin_acp_url']}/images/icon_components/blog/blog.png' alt='' style='vertical-align:bottom' />
	<a href='{$this->settings['base_url']}&amp;core&amp;module=applications&amp;section=blog' style='text-decoration:none'>IP.Blog</a>
</div>
EOF;
	}
	
	if( !IPSLib::appIsInstalled( 'gallery', false ) )
	{
		$IPBHTML .= <<<EOF
<div class='menulinkwrap'>
	<img src='{$this->settings['skin_acp_url']}/images/icon_components/gallery/gallery.png' alt='' style='vertical-align:bottom' />
	<a href='{$this->settings['base_url']}&amp;core&amp;module=applications&amp;section=gallery' style='text-decoration:none'>IP.Gallery</a>
</div>
EOF;
	}

	if( !IPSLib::appIsInstalled( 'downloads', false ) )
	{
		$IPBHTML .= <<<EOF
<div class='menulinkwrap'>
	<img src='{$this->settings['skin_acp_url']}/images/icon_components/downloads/downloads.png' alt='' style='vertical-align:bottom' />
...
EOF;
	...
	if( !$this->settings['ips_cp_purchase'] )
	{
		$IPBHTML .= <<<EOF
<div class='menulinkwrap'>
	<img src='{$this->settings['skin_acp_url']}/_newimages/icons/package.png' alt='' style='vertical-align:bottom' />
...
$IPBHTML .= <<<EOF
					...
					<td style='width: 5%; text-align: center;'>
						<a href='{$this->settings['base_url']}{$this->form_code}&amp;do=toggle_app&amp;app_id={$app['app_id']}' title='{$this->lang->words['toggle_app_enabled']}'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/{$app['_app_enabled']}' alt='-' /></a>
					</td>
					<td style='width: 5%; text-align: center;'>
	...
	<table class='alternate_rows' width='100%'>
	 ...
	 <div class='acp-actionbar'>
	 	<div class='rightaction'>
			<img src='{$this->settings['skin_acp_url']}/_newimages/icons/tick.png' alt='{$this->lang->words['a_continue']}' title='{$this->lang->words['a_continue']}' />
...
$leSigh = ( $this->request['checkUpdates'] ) ? ' class="closed"' : '';
$leBigh = ( $this->request['checkUpdates'] ) ? 0 : 1;
...
$HTML .= <<<EOF
		...
		<li{$leSigh}><a href='{$this->settings['base_url']}&amp;{$this->form_code}&amp;do=hooks_overview&amp;checkUpdates={$leBigh}'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/cog.png' /> {$this->lang->words['hook_check_updates']}</a></li>
EOF;

if( is_array( $cache ) && count( $cache ) )
{
$HTML .= <<<EOF
		<li><a href='{$this->settings['base_url']}&amp;core&amp;module=applications&amp;section=hooks&amp;do=reenable_all_hooks' style='text-decoration:none'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/cog.png' alt='' /> {$this->lang->words['hook_reenable_all']}</a></li>
EOF;
}
else
{
$HTML .= <<<EOF
		<li><a href='{$this->settings['base_url']}&amp;core&amp;module=applications&amp;section=hooks&amp;do=disable_all_hooks' style='text-decoration:none'><img src='{$this->settings['skin_acp_url']}/_newimages/icons/cog.png' alt='' /> {$this->lang->words['hook_disable_all']}</a></li>
EOF;
}
$HTML .= <<<EOF
	</ul>
...
<ul id='tabstrip_hookOverview' class='tab_bar no_title'>
	<li id='tabtab-HookInstalled'>{$this->lang->words['a_installedhooks']}</li> 
	<li id='tabtab-HookDisabled'>{$this->lang->words['a_disabledhook']}</li> 
</ul> 
...
{
ipbAcpTabStrips.register('tabstrip_hookOverview');
ipbAcpTabStrips.doToggle($('tabtab-HookInstalled'));
});
 //]]>
</script>

<div class='acp-box'>
<div id='tabpane-HookInstalled'>
	<div>
		<table width='100%' class='double_pad'>
			<tr>
				<td class='tablesubheader' style='width: 2%'>&nbsp;</td>
...
if( count( $installedHooks ) )
{
	...
	{
		$_se = ( isset( $r['hook_update_available'] ) AND $r['hook_update_available'] ) ? ' _amber' : '';
		
		$HTML .= <<<EOF
			<li class='isDraggable{$_se}' style='width:100%;' id='hook_{$r['hook_id']}'>
...
<div id='tabpane-HookDisabled'>
	<table class='alternate_rows' width="100%">
		<tr>
			<th width="50%">{$this->lang->words['a_hook']}</th>
			<th width="20%" style='text-align: center;'>{$this->lang->words['a_author']}</th>
			<th width="10%" style='text-align: center;'>{$this->lang->words['a_uptodate']}</th>
			<th width="10%" style='text-align: center;'>{$this->lang->words['a_version']}</th>
			<th width="10%">&nbsp;</th>
		</tr>
EOF;
	...
	foreach( $uninstalledHooks as $r )
	{
		$HTML .= <<<EOF
				<tr>
					...
					<td align="center" width="20%">{$r['hook_author']}</td>
...
<ul id='tabstrip_hookform' class='tab_bar no_title'>
	<li id='tabtab-HookInfo'>{$this->lang->words['hook_form_info']}</li> 
	<li id='tabtab-HookReq'>{$this->lang->words['a_hookrequirements']}</li> 
	<li id='tabtab-HookFiles'>{$this->lang->words['a_hookfiles']}</li> 
</ul> 
...
document.observe("dom:loaded",function() 
{
ipbAcpTabStrips.register('tabstrip_hookform');
ipbAcpTabStrips.doToggle($('tabtab-HookInfo'));
});
 //]]>
</script>
...
EOF;
			...
			if( count( $files ) )
			{
				foreach( $files as $index => $file )
				{
					...
					$hook_type_data			= $file['hook_type']	== 'dataHooks' ? " selected='selected'" : '';
					$hook_type_library		= $file['hook_type']	== 'libraryHooks' ? " selected='selected'" : '';

					$latestIndex			= $index > $latestIndex ? $index : $latestIndex;

					$HTML .= <<<EOF
			<div class='tablesubheader'>{$this->lang->words['a_hookfile']} #{$index}</div>
...
$HTML .= <<<EOF
				<li>
					...
					<select name='hook_type[{$index}]' id='hook_type[{$index}]' onchange='selectHookType({$index});'>
						...
						<option value='templateHooks'{$hook_type_template}>{$this->lang->words['a_templatehook']}</option>
						<option value='dataHooks'{$hook_type_data}>{$this->lang->words['a_datahook']}</option>
						<option value='libraryHooks'{$hook_type_library}>{$this->lang->words['a_libraryhook']}</option>
					</select>
				</li>
EOF;
				...
				<li id='tr_classToOverload[{$index}]'>
					<label>{$this->lang->words['a_datalocation']}</label>
...
EOF;
				...
				else if( $file['hook_type'] == 'libraryHooks' )
				{
					...
					$_applications	= $this->registry->output->formDropdown( "libApplication[{$index}]", $apps, $file['hook_data']['libApplication'] );
				...
				<li id='tr_hookApplication[{$index}]'>
					<label>{$this->lang->words['a_hookapp']}</label>
					{$_applications}
				</li>
EOF;
				}
...
<form name='theForm' method='post' action='{$this->settings['base_url']}{$this->form_code}' id='mainform'>
	...
	<div class='acp-box'>
		...
		<table class='alternate_rows' width='100%'>
			<tr><th>{$this->lang->words['a_css_info']}</th></tr>
			<tr>
				<td valign='top'>
					<p id='MF__css'>{$hookData['hook_extra_data']['display']['css']}</p>
					<div id='MF__css_popup' class='form_help' style='width:auto;text-align:center;cursor:pointer'>{$this->lang->words['a_addcss']}</div>
				</td>
	 		</tr>
	 	</table>
		<script type='text/javascript'>
